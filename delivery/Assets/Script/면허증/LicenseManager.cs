...
public class LicenseInfoManager : MonoBehaviour
{
    ...
    // --- [수정 및 추가된 변수] ---
    public bool isForgery { get; private set; }      // 전체 위조 여부 (CompareMenu에서 참조) [cite: 2026-02-12]
    public bool isPhotoForgery { get; private set; } // 사진 위조 여부 (CharacterWatcher에서 참조)
    // ----------------------------
    ...
    {
        int errorCount = Random.Range(1, 3);
        List<int> targetFields = new List<int> { 0, 1, 2, 3, 4 };
