...
public class MenuManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Kraken 카드가 special_menus에 존재하는지 확인
        if (krakenIndex == -1)
        {
    ...
    public void random_card()
    {
        ...
        {
            Debug.LogWarning("배치할 카드 프리팹이 부족하거나, 'craken' 카드를 찾을 수 없습니다.");
            return;
        }
        ...
        // 실제 배치할 카드 수 조정:
        // cardsToDisplay는 Kraken 카드 포함한 총 배치 수
        // Kraken을 제외한 나머지 카드는 num.Length(랜덤으로 선택할 수 있는 카드 수) 내에서 선택
        int actualCardsToDisplay = Mathf.Min(cardsToDisplay, num.Length + 1); // +1은 Kraken 카드를 위함
        if (actualCardsToDisplay == 0)
        {
            Debug.LogWarning("표시할 카드가 없습니다 (cardsToDisplay 설정 또는 사용 가능한 카드 부족).");
            return;
        }
        // 만약 cardsToDisplay가 1개인데 그게 kraken이 아니라면 경고
        if (actualCardsToDisplay == 1 && (cardsToDisplay > 1 || num.Length == 0)) // num.Length == 0은 craken만 있다는 의미
        {
            Debug.LogWarning("cardsToDisplay가 1이며, Kraken만 배치됩니다.");
        }
        ...
        // 최종 배치될 카드들의 순서를 섞음 (Kraken이 랜덤 위치에 놓이도록)
