...
public class LicenseJudge : MonoBehaviour
{
    ...
    public void AcceptDelivery()
    {
        ...
        Order_check currentOrder = new Order_check();
        // 위조 여부 확인
        ...
        if (isForgery)
        {
            ...
            GameManager.Instance.PendingIsDeliverySuccess.Add(false);
        ...
        else
        {
            ...
            GameManager.Instance.PendingIsDeliverySuccess.Add(true);
            Debug.Log("<color=green>배달 성공:</color> 정상 배달원 확인.");
            CheckMenu.selectedNames.Clear();
        }
        OrderChecker.Check(currentOrder);
        if (!currentOrder.isSuccess)
        {
            Debug.Log($"판정 결과: 실패 - {currentOrder.failReason}");
        }
